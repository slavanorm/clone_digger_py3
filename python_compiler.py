...
class PythonCompilerSourceFile(SourceFile):
    ...
    def __init__(self, file_name, func_prefixes=()):
        ...
        def rec_build_tree(
            compiler_ast_node, is_statement=False
        ):
            ...
            def add_leaf_child(child, name):
                ...
                assert not isinstance(child, ast.AST)
            ...
            def add_leaf_childs(childs, name):
                assert type(childs) == type([]) or type(
                    childs
                ) == type((0,))
            ...
            if isinstance(compiler_ast_node, ast.AST):
                name = compiler_ast_node.__class__.__name__
                if name == "Function":
                    for prefix in self._func_prefixes:
                        if compiler_ast_node.name.startswith(
                            prefix
                        ):
                ...
                if name == "AssAttr":
                    ...
                    add_leaf_child(
                        compiler_ast_node.attrname, "attrname"
                    )
                    add_leaf_string_childs(
                        [compiler_ast_node.flags]
                    )
                elif name == "AssName":
                    add_leaf_child(
                        compiler_ast_node.name, "name"
                    )
                #                   add_leaf_child(compiler_ast_node.flags, 'flags')
                    ...
                    for i in range(len(compiler_ast_node.ops)):
                        ...
                        add_childs([expr])
                        compiler_ast_node.ops[i] = (
                            t.ast_node,
                            expr,
                        )
                elif name == "Const":
                    add_leaf_child(
                        repr(compiler_ast_node.value), "value"
                    )
                #               elif name == "From":
                ...
                #                   add_childs(compiler_ast_node.names)
                    ...
                    add_leaf_childs(
                        compiler_ast_node.argnames, "argnames"
                    )
