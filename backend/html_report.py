...
class CPDXMLReport(Report):
    ...
    def writeReport(self, file_name):
        ...
        for clone in self._clones:
            token_numbers = [
                sum([s.getTokenCount() for s in clone[i]])
                for i in (0, 1)
            ]
            f.write(
                '<duplication lines="'
            ...
            for i in [0, 1]:
                f.write(
                    ...
                    + str(
                        1
                        + min(clone[i].getCoveredLineNumbers())
                    )
                    + '" path="'
                    + os.path.abspath(
    ...
    def writeReport(self, file_name):
        ...
        for clone_i in range(len(self._clones)):
            try:
                ...
                s += (
                    "Distance between two fragments = %d <BR>"
                    % (clone.calcDistance())
                )
                s += "Clone size = " + str(
                    max(
                ...
                for i in range(clone[0].getLength()):
                    ...
                    for j in (0, 1):
                        for source_line in statements[
                            j
                        ].getSourceLines():
                            indentations[j].add(
                                re.findall("^\s*", source_line)[
                                    0
                                ].replace("\t", 4 * " ")
                            )
                    indentations = (
                    ...
                    def use_diff():
                        for j in (0, 1):
                            for source_line in statements[
                                ...
                                indent2 = indent1.replace(
                                    "\t", 4 * " "
                                )
                                # todo index seems broken
                                source_line = re.sub(
                                    "^" + indent1,
                                    indentations[j].index(
                                        indent2
                                    )
                                    * " ",
                                    source_line,
                                )
                                source_lines[j].append(
                                    source_line
                                )
                        d = diff_highlight(
                            [
                    ...
                    if global_settings.use_diff:
                        ...
                        try:
                            ...
                            rec_correct_as_string(
                                ...
                                s2,
                                list(
                                    u.getSubstitutions()[0]
                                    .getMap()
                                    .values()
                                ),
                                list(
                                    u.getSubstitutions()[1]
                                    .getMap()
                                    .values()
                                ),
                            )
                    ...
                    s += (
                        t[0]
                        + '<TD style="width: 10px;" BGCOLOR=%s> </TD>'
                        % (color,)
                        + t[1]
                    )
                    s += "</TR>\n"
                ...
                print(
                    "Clone info can't be written to the report. "
                )
                traceback.print_exc()
...
clusterize_using_dcup = %s<BR>
        ...
        if self._mark_to_statement_hash:
            ...
            for mark in marks[:20]:
                ...
                marks_report += (
                    '<div id="stmt%d" class="hidden"> <BR>'
                    % (counter,)
                )
                for statement in self._mark_to_statement_hash[
                    mark
                ]:
        ...
        save_to = (
            eclipse_start
            + '<b><a href="file://%s">Save this report</a></b>'
            % (file_name,)
            + eclipse_end
        )
        HTML_code = """
